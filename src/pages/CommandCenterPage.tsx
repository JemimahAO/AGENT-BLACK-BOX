    ...
    <AppLayout>
      <div className="p-4 md:p-6 space-y-4 md:space-y-5">
              ...
              <div className="flex items-start gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-black text-destructive uppercase tracking-widest">⬤ Critical Incident — Active</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-1.5">
                    <span className="text-[10px] mono text-muted-foreground border border-border bg-muted/50 px-1.5 py-0.5 rounded">INC-2026-06-09-00073</span>
                    <span className="text-[10px] mono text-primary">SalesDataAgent v3.1.0</span>
                    <span className="text-[10px] text-muted-foreground">Investigation Open · 2 min ago</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center md:flex-col lg:flex-row shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-bold text-foreground mono">2m ago</div>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <MetricCard
            title="Agent Runs"
            value={metrics.totalAgentRuns.toLocaleString()}
            delta={`▲ ${metrics.totalAgentRunsDelta}% vs last 7 days`}
            icon={Activity}
          ...
          <MetricCard
            ...
            value={metrics.highRiskRuns}
            delta={`▼ ${Math.abs(metrics.highRiskRunsDelta)}% vs last 7 days`}
            icon={Shield}
          ...
          <MetricCard
            ...
            value={metrics.blockedActions.toLocaleString()}
            delta={`▲ ${metrics.blockedActionsDelta}% vs last 7 days`}
            icon={Lock}
              ...
              <div className="mt-2 space-y-1.5">
                {mockRiskDistribution.map((r) => (
                  <div key={r.name} className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-foreground font-semibold mono">{r.percentage}% ({r.value.toLocaleString()})</span>
                  </div>
                ))}
              </div>
            </div>
