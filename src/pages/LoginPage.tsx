...
export default function LoginPage() {
  ...
  const { login, continueAsDemo } = useSession();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success('Signed in', { description: 'Welcome to AgentBlackbox' });
  ...
  const handleContinueAsDemo = () => {
    continueAsDemo();
    toast.success('Demo session started', { description: 'Signed in as Jemimah Adwar (Demo Admin)' });
    navigate('/command-center');
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 md:py-0">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-foreground">AgentBlackbox</h1>
            <p className="text-sm text-muted-foreground">The event ledger for AI agents.</p>
          </div>
          ...
          <Button
            ...
            className="w-full bg-status-low/15 border border-status-low/35 text-status-low hover:bg-status-low/25 font-bold py-2.5 h-auto"
          >
